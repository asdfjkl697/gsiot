...
Statement::Statement(Database &aDatabase, const char* apQuery) : // throw(SQLite::Exception)
		mQuery(apQuery), mStmtPtr(aDatabase.mpSQLite, mQuery), // prepare the SQL query, and ref count (needs Database friendship)
		mColumnCount(0), mbOk(false), mbDone(false) {
...
void Statement::bind(const int aIndex, const std::string& aValue) // throw(SQLite::Exception)
		{
	int ret = sqlite3_bind_text(mStmtPtr, aIndex, aValue.c_str(), aValue.size(),
			SQLITE_TRANSIENT);
	check(ret);
}
...
void Statement::bind(const int aIndex, const char* apValue) // throw(SQLite::Exception)
		{
	int ret = sqlite3_bind_text(mStmtPtr, aIndex, apValue, -1,
			SQLITE_TRANSIENT);
	check(ret);
}
...
void Statement::bind(const int aIndex, const void* apValue, const int aSize) // throw(SQLite::Exception)
		{
	int ret = sqlite3_bind_blob(mStmtPtr, aIndex, apValue, aSize,
			SQLITE_TRANSIENT);
	check(ret);
}
...
void Statement::bind(const char* apName, const std::string& aValue) // throw(SQLite::Exception)
		{
	int index = sqlite3_bind_parameter_index(mStmtPtr, apName);
	int ret = sqlite3_bind_text(mStmtPtr, index, aValue.c_str(), aValue.size(),
			SQLITE_TRANSIENT);
	check(ret);
}
...
void Statement::bind(const char* apName, const void* apValue, const int aSize) // throw(SQLite::Exception)
		{
	int index = sqlite3_bind_parameter_index(mStmtPtr, apName);
	int ret = sqlite3_bind_blob(mStmtPtr, index, apValue, aSize,
			SQLITE_TRANSIENT);
	check(ret);
}
...
Statement::Ptr::Ptr(sqlite3* apSQLite, std::string& aQuery) :
		mpSQLite(apSQLite), mpStmt(NULL), mpRefCount(NULL) {
	int ret = sqlite3_prepare_v2(apSQLite, aQuery.c_str(), aQuery.size(),
			&mpStmt, NULL);
	if (SQLITE_OK != ret) {
...
Statement::Ptr::Ptr(const Statement::Ptr& aPtr) :
		mpSQLite(aPtr.mpSQLite), mpStmt(aPtr.mpStmt), mpRefCount(
				aPtr.mpRefCount) {
