...
std::string g_GetConnectStateStr( defConnectState state )
{
	...
	case defConnectState_Disconnected:
		return std::string("连接断开");
	}

	return std::string("其它");
}
...
std::string g_getstr_Useable( const defUseable Useable )
{
	...
	case defUseable_ErrConfig:
		return std::string("配置错误");
		break;

	case defUseable_ErrNoData:
		return std::string("有异常无数据");
		break;

	case defUseable_ErrNet:
		return std::string("连接中断");//return std::string("有异常连接中断");
		break;

	default:
		if( Useable < 0 ) return std::string("有异常");
		break;
	}
...
std::string g_StrToPrecision( const std::string &srcstr, const int precision )
{
	...
	const size_t getpos = pos + precision + (precision>0?1:0); // 包括"."
...
std::string g_SecondToStr( const uint32_t second )
{
	...
	//if( h>0 )
	//{
	//	snprintf( chtime, sizeof( chtime ), "%d时", h );
	//	str += chtime;
	//}

	if( m>0 )
	{
		snprintf( chtime, sizeof( chtime ), "%d分", m );
		str += chtime;
	}

	snprintf( chtime, sizeof( chtime ), "%d秒", s );
	str += chtime;
