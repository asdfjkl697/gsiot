...
void *CommLink_Thread(LPVOID lpPara) //jyc20160826 test
{
	...
	const bool allowConnect = ( CommLinkThread_allowConnectNum<CommLinkThreadMaxAllow || CommLinkThread_allowConnectNum<1 ) && this_CommLinkThread_No%2; // 至少一个；奇数允许，偶数不允许，避免全部允许的情况；
	if( allowConnect )
	{
				...
				case defCommLinkHeartbeatType_SpecRdAddr:
					{
						...
						// 获取之前的心跳设备并判断是否与配置一致，是否需要重建心跳设备
...
bool DeviceConnection::SetRFMod_EncodeSend( defLinkID LinkID, defFreq freq, uint8_t enable_pass, uint8_t check_count, uint8_t enable_timecheck )
{
	...
	sendBuf[0] = 0xBC;
	sendBuf[1] = 0xAC;
	uint8_t *pPacketLen = sendBuf+2;		// 包长度 位置
	uint8_t *pPacketVer = pPacketLen+1;		// 包版本 位置
	uint8_t *pModuleIndex = pPacketVer+1;	// 模块索引 位置
	uint8_t *enc = pModuleIndex+1;			// 数据段编码区
	uint8_t *pPacketEnd = NULL;				// 结束确认位 位置
...
bool DeviceConnection::SendMOD_set_EncodeSend( const defMODSysSet cmd, const defLinkID LinkID, const uint32_t QueueOverTime )
{
	...
	case defMODSysSet_IR_RXMod_original:	// IR 收原始数据
		{
	...
	case defMODSysSet_IR_RXMod_code:		// IR 收编码数据
		{
...
bool DeviceConnection::Encode_RFRemote_original( const RFSignal &signal, uint8_t *buf, uint16_t &buflen )
{
	...
	*enc = signal_count; enc++;
	...
	// 学习到的数据基础上，引导码为0x00负脉冲时，最后需补上引导码时间
	if( 0 == signal.original_headflag )
	{
		memcpy( enc, &signal.original_headtime, sizeof(uint16_t) );
		enc += sizeof(uint16_t);
...
bool DeviceConnection::Encode_RFRemote_IR_code( const RFSignal &signal, uint8_t *buf, uint16_t &buflen )
{
	...
	*enc = signal_count; enc++;
	...
	const int originalbytelen = signal.original_len <= sizeof(signal.original) ? signal.original_len : sizeof(signal.original); // original_len=字节
...
// 分包内容大小
#define defSendDataClipSize	(255-6)  // 最大减去头尾
...
int DeviceConnection::SendData(CCommLinkRun *CommLink, unsigned char *sendbuf,int sendsize)
{
	...
	memcpy( clipbuf, sendbuf, 5 ); // 头内容
	uint8_t *const clipbuf_PacketLen = clipbuf+2;	// 包长度 位置
	uint8_t *const clipbuf_data = clipbuf+5;		// data 位置

	unsigned char *buf_temp = sendbuf+5; // 当前数据动态地址
	int buf_temp_size = sendsize-6; // 当前数据动态剩余大小
	const unsigned char *const buf_temp_end = sendbuf+sendsize-1; // 数据结尾
	//int buf_temp_size = buf_temp_end-buf_temp; // 当前数据动态剩余大小
	...
	for( ; buf_temp<buf_temp_end; )
	{
		...
		*clipbuf_PacketLen = cur_copy_size+6-MIAN_PORT_UNPkt_LEN;// 大小
		clipbuf_data[cur_copy_size] = 0xAC; // 结尾
...
if( IsRUNCODEEnable(defCodeIndex_TEST_ModbusMod) )
	...
	while(size>5)
	{
		uint8_t PacketHead = *buffer++; size--;			// 包头
		uint8_t PacketHeadCheck = *buffer++; size--;	// 包头 确认位
		uint8_t PacketLen = *buffer++; size--;			// 包长度 位置
		uint8_t PacketVer = *buffer++; size--;			// 包版本 位置
		uint8_t ModuleIndex = *buffer++; size--;		// 模块索引 位置
		...
		uint8_t DataLen = PacketLen-2;			// 数据段长度
		if( size<(uint32_t)(DataLen+1) ) // 数据段和结束确认位 的长度
		{
		...
		uint8_t PacketEnd = *(buffer+DataLen);	// 结束确认位 位置
		if( 0xAC != PacketEnd )
		{
...
void DeviceConnection::DecodeRxb8Data( CHeartbeatGuard *phbGuard, CCommLinkRun *CommLink, defFreq freq, uint8_t* const srcbuf, const uint32_t srcbufsize )
{
	...
	// 根据指令头尾定义，取出对应位的值
	// 引导码长度
	if( buf[0] & 0x01 ) // 是否有引导码
	{
		signal.headlen = (buf[0] & 0x0E)>>1;
	}

	// 结束码长度
	if( buf[0] & 0x10 ) // 是否有结束码
	{
...
void DeviceConnection::Decode_RS485Data( CHeartbeatGuard *phbGuard, CCommLinkRun *CommLink, uint8_t* const srcbuf, const uint32_t srcbufsize )
{
	...
	if( !CurCmd )
	{
		...
		{
			this->m_handler->OnDeviceData( LinkID, NULL, pDecodeCtrl, NULL ); // 只通知地址不通知值，用于遍历检测
		}
...
void DeviceConnection::Decode_Rxb8Data_original( CHeartbeatGuard *phbGuard, CCommLinkRun *CommLink, const defFreq freq, uint8_t* const srcbuf, const uint32_t srcbufsize )
{
	...
	// 引导码正脉冲或负脉冲	(1 byte)	0x00负脉冲，0x01正
	...
	case 0x30:
		{
			...
			case 0x0D:
			case 0x1D:
				pMsgCurCmd->Set_MODSysSetInfo( defMODSysSetInfo_RXMod, *buf );	// 数据格式 原始或编码
				break;

			case 0x0A:
			case 0x1A:
				pMsgCurCmd->Set_MODSysSetInfo( defMODSysSetInfo_TXCtl, *buf );	// 控制模式 接收(学习模式)或发送
				break;
