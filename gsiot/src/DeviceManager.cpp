	...
	:SQLiteHelper()
{
	...
	while(rf_m_query.executeStep()){
	    ...
	    while(addr_query.executeStep()){
			...
			int col = 4;
			//if( !addr_query.isColumnNull(col) ) { addr->SetName((std::string)addr_query.getColumn(col)); } col++;
			...
			//addr->SetDefualtValue((std::string)addr_query.getColumn(col++));
			//addr->SetMinValue((std::string)addr_query.getColumn(col++));
			//addr->SetMaxValue((std::string)addr_query.getColumn(col++));
...
bool DeviceManager::LoadDB_remote_button( const IOTDeviceType device_type, const int device_id, RemoteButtonManager *pRemoteButtonManager, IOTDeviceType ExType )
{
	...
	while(query.executeStep())
	{
		...
		std::string name;
		//if( !query.isColumnNull(col) ) { name = (std::string)query.getColumn(col); } col++;
