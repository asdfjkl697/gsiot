...
bool GSIOTConfig::PreInit( const std::string &RunParam )
{
	...
	if (query.executeStep()){
		...
		//if( !query.isColumnNull(9) ) { this->m_phone = query.getColumn(9); }
		if( !query.isColumnNull(10) ) { this->m_DoInterval_ForSMS = (uint32_t)query.getColumn(10).getInt(); }
	}
...
bool GSIOTConfig::ClearAllRelation_SpecType( IOTDeviceType device_type )
{
	if( !m_mapRelation.empty() )
	{
		for( defmapRelation::const_iterator it = m_mapRelation.begin(); it != m_mapRelation.end(); )
		{
			if( it->first.device_type == device_type )
			{
				//20160604 m_mapRelation.erase(it); 
