	...
	if( tmgr )
	{
		...
		if( tstate_alarmguard )
		{
			...
			for( ; it != lw.end(); ++it )
			{
				...
				//.resetallday
				//if( defAlarmGuardTime_AllDay != this->m_mapAGTime[w].allday && defAlarmGuardTime_UnAllDay != this->m_mapAGTime[w].allday )
				//{
				//	if( allinvalid )
				//	{
				//		LOGMSG( "ad invalid, and reset ad=0(AllDay)" );
				//		this->m_mapAGTime[w].allday = defAlarmGuardTime_AllDay;
				//	}
				//}
			}
		}
...
Tag* XmppGSState::tag() const
{
	...
	for( int nW=1; nW<=7; ++nW )
	{
		...
		{
			if( !vecFlag[i] && vecBegin[i] == vecEnd[i] ) // 标志无效并且值无效的不上传，只要值有效不管标志是否有效都上传
			{
				continue;
			}
