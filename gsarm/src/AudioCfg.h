...
{
	 defAudioSource_Null		= 0, // 无音频，即使摄像头本身有音频，也丢弃不要
	 defAudioSource_Camera		= 1, // 摄像头音频
	 defAudioSource_LocalCap	= 2, // 本地采集音频
...
{
	defAudioParamDef_SysDefault		= 0, // 采用系统默认
	defAudioParamDef_UserDefine		= 1, // 用户自定义
	defAudioParamDef_Analyse		= 2, // 分析音频源
};
...
private:
	defAudioSource_ m_Audio_Source;		// 音频源
	defAudioFmtType_ m_Audio_FmtType;	// 音频格式
	defAudioParamDef_ m_Audio_ParamDef;	// 参数定义
	int m_Audio_Channels;	// 音频声道 1 or 2
	int m_Audio_bitSize;	// bit 音频比特位 8 or 16
	int m_Audio_SampleRate;	// 音频采样频率, 5512Hz, 8000Hz, 11025HZ, 22050HZ, 44100HZ 等等
	int m_Audio_ByteRate;	// 字节速率

	//int m_Audio_Info1;		// 预留 音频信息1
	//int m_Audio_Info2;		// 预留 音频信息2
	std::string m_Audio_cap_src;	// 音频本地采集源的音频输入设备名称
	std::string m_Audio_cap_param;	// 音频本地采集其它参数
