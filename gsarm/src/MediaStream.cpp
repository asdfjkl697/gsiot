...
void MediaStream::SetVideoNal(x264_nal_t *nal,int i_nal)
{
	char chbuf[256] = {0};
...
{
	PushVideo(keyframe, data,size, 0, false, isAudio);
}

void MediaStream::PushVideo( const bool keyframe, char *data, int size, uint32_t timestamp, const bool use_input_ts, const bool isAudio )
{
	if(m_publish || this->GetRTMPSendObj()->GetIPlayBack()){
		if( use_input_ts )
			m_rtmp->PushVideo(keyframe, data,size, timestamp, isAudio);
		else
