	...
	case defAudioFmtType_AAC:
		{
			Audio_Channels = 1;
			Audio_bitSize = 16;
			Audio_SampleRate = 22050;//44100
			Audio_ByteRate = 4 * defByteRate_base_k;
		}
		break;
...
const std::string CAudioCfg::getstr_AudioChannels( const int Audio_Channels )
{
	...
	{
	case 1:
		return std::string("单声道");

	case 2:
		return std::string("双声道");
	}
...
bool CAudioCfg::isChanged( const CAudioCfg &AudioCfg ) const
{
	...
	if( defAudioSource_Null == this->m_Audio_Source ) // 都是null没有发生变化
		return false;
