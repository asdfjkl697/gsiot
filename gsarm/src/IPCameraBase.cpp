...
defGSReturn IPCameraBase::SendToRTMPServer( const JID& jid, const std::string& url, const std::vector<std::string> &url_backup )
{
	if( !this->IsConnect() )
	{
		...
		{
			this->SetFrameState( true ); // Ԥ��λ
		}
	}

	// �ȸ��»Ự��Ȼ���飬���֮ǰ���Ѿ���������ô���º�Ӧ�������������Ự
	bool isAdded = false;
	...
	//--------------------------
	// ����δ�����ģ���ݷ�������url��������
	// �����ѷ����ģ��ɵ�ΪRTMFP���µ�ΪRTMP�����RTMP�����ӣ����߹��棬RTMP����ʹ�ã����򲻽����µ�����
	// Ч��δ���������ǽ����µ����ӣ��ѷ���RTMP���ٽ��������ӣ��ѷ���RTMFP������µ�RTMP���ӣ���RTMFP�Ա�������Ͽ�����RTMP���棬��RTMP����ʹ�á�

	const bool Old_isRTMFP = g_IsRTMFP_url( this->getUrl() );	// �����Ѿ����ڵ�url�Ƿ���RTMFP
	const bool New_isRTMFP = g_IsRTMFP_url( useUrl );			// Զ�̷���������url�Ƿ���RTMFP

	bool doConnect_RTMP = false; // �Ƿ����RTMP����
	bool doConnect_RTMFP = false; // �Ƿ����RTMFP����
	...
	if( doConnect_RTMFP )
	{
		...
		if( stream->GetRTMPSendObj()->getPeerID().empty() )
		{
			...
			doConnect_RTMP = true; // ����RTMFPʧ�ܺ��Զ���������RTMP
		}
		else
		{
...
bool IPCameraBase::CheckSession( const uint32_t enable, const bool isPlayBack, const bool CheckNow )
{
	if( CheckNow )
	{
	...
	const bool isState_Publish = IsPublish() && this->stream->GetRTMPSendObj()->IsRtmpLive();
	...
	// ֻҪ���ڲ�һ�¾�ֹͣ
	if( isState_Publish != isState_Url
		|| isState_Url != isState_Session 
	...
	if( doDisconnect )
	{
		...
		if( enable 
			&& (isAutoConnectFlag || isState_Session)
			)
		{
			doConnect = true;
		}
...
void IPCameraBase::PushVideo( const bool keyframe, const uint32_t rate, char *data, int size, uint32_t timestamp, const bool use_input_ts, const bool isAudio )
{
#if 0
	// ������·��Ƶ����s_audio_testdest��
	static IPCameraBase *s_audio_testdest = NULL;
	static IPCameraBase *s_audio_testsrc = NULL;
	if( s_audio_testdest )
	{
		IPCameraBase *s_audio_testdest_temp = s_audio_testdest;
		if( isAudio && s_audio_testdest_temp != this )
		{
			if( !s_audio_testsrc )
				s_audio_testsrc = this;

			if( s_audio_testsrc == this )
			{
				s_audio_testdest_temp->PushVideo( keyframe, rate, data, size, timestamp, use_input_ts, isAudio );
			}
		}

		if( s_audio_testdest == this )
		{
			if( !this->GetStreamObj()->GetRTMPSendObj()->IsRunning() )
			{
				s_audio_testdest = NULL;
			}
		}
	}
	else
	...
	if( this->GetStreamObj()->GetRTMPSendObj()->GetIPlayBack() )
	{
		//if( !this->GetStreamObj()->GetRTMPSendObj()->get_playback_sound() )
		//{
		//	return;
		//}
	...
	if( use_input_ts )
	{
		...
		this->stream->PushVideo( keyframe, data, size, timestamp, use_input_ts, isAudio );
		return ;
	}
...
#if 0
	cur_ts = timeGetTime() - this->m_state.startTime;
#else
	cur_ts = timeGetTime();
#endif
	...
	//stream->PushVideo(pFrame->nPacketType == VIDEO_I_FRAME,buffer+4, size-4); //h264 no head
