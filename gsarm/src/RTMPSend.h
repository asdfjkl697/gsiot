...
{
	uint32_t index; // 包序列号，循环递增
	int bufLimit;	// 缓存最大可用空间大小
    char* buf;
	bool keyframe;
    uint32_t timeStamp;
    int size;		// 当前缓存数据内容大小
...
private:	
	...
	uint32_t m_doSkipTime_timems; // GSPlayBackCode_SkipTime时有效
	int m_PlayBackCtrl_speedlevel; // GSPlayBackCode_PLAYFAST/GSPlayBackCode_PLAYSLOW时有效
	uint32_t m_PlayBackCtrl_ThrowFrame; // GSPlayBackCode_PLAYFAST时有效
	GSPlayBackCode_ m_PlayBackCtrl_pause_oldcode; // GSPlayBackCode_PLAYPAUSE时有效
...
public:
	...
	*/
	...
	void SetVideoNal(x264_nal_t *nal,int i_nal);
