...
struct struCamAlarmRecv
{
	...
	IPCameraType CameraType;	// 摄像机类型
	std::string alarmstr;		// 告警内容
...
typedef std::list<struCamAlarmRecv> deflstCamAlarmRecv;// 摄像机告警原始信息接收map类型定义
...
struct struPlayMgrCmd
{
	...
	int dev_id;
	std::string url;
	std::vector<std::string> url_backup; // 备份地址
