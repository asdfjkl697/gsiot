...
{
	IOTDeviceType child_dev_type;	// 关联设备类型
	int child_dev_id;				// 关联设备ID
	int child_dev_subid;			// 关联子对象ID  0 表示所有子对象
...
public:
	...
	bool isOwner( const std::string &UserJID ); // 设备所属帐号，拥有这台设备的帐号
	bool isOwnerForKeyJid( const std::string &OwnerKeyJid, const std::string &UserKeyJid ) // 设备所属帐号，拥有这台设备的帐号
	{
...
private:
	...
	std::string m_phone;
	uint32_t m_DoInterval_ForSMS; // 执行间隔，单位秒

private:
	defmapRelation m_mapRelation; // 设备关联
	defmapVObjConfig m_mapVObj;		// 虚拟对象
