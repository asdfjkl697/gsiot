...
{ \
	...
	{ \
		LOGMSGEX( defLOGNAME, defLOG_ERROR, "s_InitRunCode: "#in_codestr" code same!!!\r\n\r\n" ); \
	} \
} \
else \
{ \
	LOGMSGEX( defLOGNAME, defLOG_ERROR, "s_InitRunCode: "#in_codestr" code error!!!\r\n\r\n" ); \
}
...
bool CRunCodeMgr::SaveDB_code( defCodeIndex_ index, bool save_str_val )
{
	...
	char sqlbuf[1024] = {0};
	sprintf_s( sqlbuf, sizeof(sqlbuf), "SELECT id FROM [code] WHERE code=%d OR codestr='%s' LIMIT 1", s_RunCode[index].code, s_RunCode[index].codestr );

	//SQLite::Statement queryid( *this->db, "SELECT id FROM [code] WHERE code=? OR codestr='?' LIMIT 1" );
	SQLite::Statement queryid( *this->db, sqlbuf );
	//int col = 1;
	//queryid.bind(col++, s_RunCode[index].code);
	//queryid.bind(col++, s_RunCode[index].codestr);
	...
	if( save_str_val )
	{
		LOGMSGEX( defLOGNAME, defLOG_INFO, "SaveDB_code success. code={%d,%s}, str_val=\"%s\"\r\n", 
			s_RunCode[index].code, s_RunCode[index].codestr, s_RunCode[index].str_val.empty()?"":s_RunCode[index].str_val.c_str() );
	}
	else
	{
		LOGMSGEX( defLOGNAME, defLOG_INFO, "SaveDB_code success. code={%d,%s}, val={%d, %d, %d, %d}\r\n", 
			s_RunCode[index].code, s_RunCode[index].codestr, s_RunCode[index].value1, s_RunCode[index].value2, s_RunCode[index].value3, s_RunCode[index].value4 );
	}
