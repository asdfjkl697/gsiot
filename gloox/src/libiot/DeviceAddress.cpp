	...
	if( annlyse )
	{
		...
		if( abs( newCurValueF-atof(m_curValue.c_str()) ) > 5.0f )
		{
			...
			else
			{
				...
				{
					char chlog[1024] = {0};
					sprintf_s( chlog, sizeof(chlog), "SetCurValue(%d,%d) data_abnormal_count=%d, old=%s, new=%s", m_type, this->GetAddress(), m_data_abnormal_count, m_curValue.c_str(), newValue.c_str() );
					*strlog = chlog;
				}
		...
		if( isFinishValue )
		{
			if( m_lastSaveTime )
			{
				...
				if( g_TransToTimePoint(newValTime, m_type, false)!=g_TransToTimePoint(m_lastSaveTime, m_type, true) )
				{
					*doSave = true;
					...
					{
						char chlog[1024] = {0};
						sprintf_s( chlog, sizeof(chlog), "ValueDoSave for TimePoint (%d,%d) old=%s, new=%s", m_type, this->GetAddress(), m_lastSaveValue.c_str(), newFinishValue.c_str() );
						*strlog = chlog;
					}
				}
					...
					case IOT_DEVICE_Wind:
						{
							...
							*dataflag = defDataFlag_Changed;
							...
							{
								char chlog[1024] = {0};
								sprintf_s( chlog, sizeof(chlog), "ValueDoSave for change (%d,%d) old=%s, new=%s", m_type, this->GetAddress(), m_lastSaveValue.c_str(), newFinishValue.c_str() );
								*strlog = chlog;
							}
						}
						break;
					...
					default:
						{
							...
							*dataflag = defDataFlag_Changed;
							...
							{
								char chlog[1024] = {0};
								sprintf_s( chlog, sizeof(chlog), "ValueDoSave for change (%d,%d) old=%s, new=%s, VChgRng=%.3f", m_type, this->GetAddress(), m_lastSaveValue.c_str(), newFinishValue.c_str(), VChgRng );
								*strlog = chlog;
							}
						}
						break;
			...
			else
			{
				...
				*dataflag = defDataFlag_First;
				...
				{
					char chlog[1024] = {0};
					sprintf_s( chlog, sizeof(chlog), "ValueDoSave for first (%d,%d) new=%s", m_type, this->GetAddress(), newFinishValue.c_str() );
					*strlog = chlog;
				}
			}
