...
public:
	...
	{
		defAttr_Min_ = 0,
		defAttr_Enable = 0,				// 启用标志
		defAttr_IsReSwitch,				// 两态翻转切换式开关
		defAttr_IsAutoBackSwitch,		// 自返回自回复开关
		defAttr_Max_
	};
...
private:
	...
	std::string m_curValue; // 可以直接大于、小于、等于比较
	...
	int m_data_abnormal_count;	// 异常计算
	int m_data_MultiReadCount;  // 连续读取次数
