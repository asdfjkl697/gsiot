...
Tag* GSIOTDevice::tag(const struTagParam &TagParam) const
{
	...
	else
	{
		...
		if( defUseable_OK != useable ) { i->addAttribute( "useable", useable ); }; // Ĭ�Ͽ��ã�������ʱ���ṩ��ֵ

		const defAlarmState AlarmState = this->GetCurAlarmState();
		if( macAlarmState_IsAlarm(AlarmState) ) { i->addAttribute( "almst", AlarmState ); }; // Ĭ����
...
{
	return ( isAlarm?std::string("ע��"):std::string("�ָ���") );
}
...
IOTDeviceReadType GSIOTDevice::getStaticReadType( const IOTDeviceType type )
{
	...
	{
		case IOT_DEVICE_CO2:// �¶�
		case IOT_DEVICE_HCHO:	// ʪ��
		//case IOT_DEVICE_PM25:
		case IOT_DEVICE_Temperature:// �¶�
		case IOT_DEVICE_Humidity:	// ʪ��
		case IOT_DEVICE_Wind:		// ����
		case IOT_DEVICE_Trigger:	// ������
		case IOT_DEVICE_Event:		// ���¼�
		case IOT_DEVICE_RFDevice:	// �����豸
			return IOT_READ;

		case IOT_DEVICE_Remote:		// ң��
			return IOT_WRITE;

		case IOT_DEVICE_Switch:		// ����
		case IOT_DEVICE_CANDevice:	// CAN�豸
		case IOT_DEVICE_Camera:
