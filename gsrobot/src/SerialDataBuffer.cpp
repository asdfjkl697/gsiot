	...
	)
{
	...
	if( address || other_address ) // ֻҪ��һ��address��ֵ�����address�Ƚ�
	{
...
std::string CMsgCurCmd::Get_MODSysSetInfo( const defMODSysSetInfo MODSysSetInfo, uint32_t *last_ts, bool commboget, int *getval )
{
	...
	case defMODSysSetInfo_TXCtl:
		...
		{
			str = m_MODSysSetInfo_TXCtl ? "���ⷢ��ģʽ":"����ѧϰģʽ";

			if( m_MODSysSetInfo_RXMod>=0 && 0==m_MODSysSetInfo_TXCtl )
			{
				str += ", ";
				str += m_MODSysSetInfo_RXMod ? "ԭʼ��ݸ�ʽ":"������ݸ�ʽ";
			}
		}
		break;
	...
	case defMODSysSetInfo_RF_RX_freq:
		...
		{
			str += defFreq_315==m_MODSysSetInfo_RF_RX_freq ? "���߽���315MHz":(defFreq_433==m_MODSysSetInfo_RF_RX_freq ? "���߽���433MHz":"");
		}
		break;
...
bool CMsgCurCmd::Is_CurCmd( ControlBase *DoCtrl )
{
	...
	if( IOT_DEVICE_RS485 != DoCtrl->GetType() ) // Ŀǰֻ����RS485
		return false;
